    ...
    $.ajax({
        ...
        success:function(respuesta){
            for(var i = 0;i<respuesta.data.length;i++){
                if (respuesta.data[i][4].length > 0 && respuesta.data[i][3].length > 0){ 
                    if(respuesta.data[i][6]=='Sin Ver'){
                        $("#ModalAlerta").attr("IdAlerta",respuesta.data[i][0]);
                        $("#TitleModalAlert").html(respuesta.data[i][4]);
                        $("#TextModalAlert").html(respuesta.data[i][5]);
                        $("#ModalAlerta").modal();
                    }                   
                    code = ""; 
                    code = "<li><a href='javascript:void(0);'>";
                    code += "<div class='menu-info'>";
                    code += "<h4>"+respuesta.data[i][4]+"</h4>";
                    code += "<p><i class='material-icons'>access_time</i>"+respuesta.data[i][3]+"";
                    code += "</p>";
                    code += "</div>";
                    code += "</a>";
                    code += "</li>";
